// src/modules/legal/privacy-notice.ts
  ...
  sections: {
    ...
    purposes: [
      ...
      },
      {
        ...
        legalBasis: 'Ejecución de contrato y cumplimiento de obligación legal — Art. 7 y 10 LFPDPPP; Ley de Voluntad Anticipada del Distrito Federal y leyes estatales equivalentes',
        required: true,
      },
      {
        id: 'emergency_qr',
      ...
      // — FINALIDADES SECUNDARIAS (no necesarias; el titular puede oponerse) —
      {
    ...
    // Art. 16 fracc. IV y Art. 36-37 LFPDPPP
    // ─────────────────────────────────────────────────────────────
    transfers: [
      {
        ...
        legalBasis: 'Art. 37 fracc. I LFPDPPP — necesaria para la atención de una emergencia que pueda dañar a una persona.',
      },
      {
        recipient: 'Representantes y contactos de emergencia designados por el titular',
        country: 'México',
        purpose:
          'Notificación de alertas de pánico, escaneos del código QR y actualizaciones de directivas.',
      ...
      },
      {
        ...
        legalBasis: 'Art. 36 LFPDPPP — transferencia internacional con medidas de seguridad equivalentes; AWS cuenta con certificaciones SOC 2, ISO 27001 y ofrece Cláusulas Contractuales Estándar.',
      },
      {
        recipient: 'Stripe, Inc.',
        country: 'Estados Unidos de América',
        purpose:
          'Procesamiento seguro de pagos con tarjeta de crédito/débito. Stripe es certificado PCI DSS Nivel 1.',
        legalBasis: 'Art. 36 LFPDPPP — ejecución del contrato de servicio; Stripe cumple con medidas de seguridad equivalentes.',
      },
      {
      ...
      },
      {
        ...
        country: 'Estados Unidos de América',
        purpose:
          'Envío de alertas y notificaciones de emergencia vía WhatsApp a los contactos designados por el titular.',
        legalBasis: 'Art. 36 LFPDPPP — consentimiento del titular; transferencia necesaria para la finalidad principal del servicio.',
      },
    ],

    // ─────────────────────────────────────────────────────────────
    // IV. DERECHOS ARCO
    // Art. 16 fracc. V y Art. 28-35 LFPDPPP
    // ─────────────────────────────────────────────────────────────
    arcoRights: {
      description:
    ...
    // Art. 8 fracc. II y Art. 16 fracc. V LFPDPPP
    // ─────────────────────────────────────────────────────────────
    revocation: {
      mechanism:
    ...
    // ─────────────────────────────────────────────────────────────
    // XI. DERECHO A PRESENTAR QUEJA ANTE EL INAI
    // Art. 45 LFPDPPP
    // ─────────────────────────────────────────────────────────────
    inapeContactInfo:
...
// ─────────────────────────────────────────────────────────────
// VERSIÓN SIMPLIFICADA — Art. 16 bis LFPDPPP
// Para pantallas pequeñas y flujo de registro
// ─────────────────────────────────────────────────────────────
export const privacyNoticeSimplified = {
  version: privacyNotice.version,
  effectiveDate: privacyNotice.effectiveDate,
  responsibleParty: {
    name: privacyNotice.responsibleParty.name,
    email: privacyNotice.responsibleParty.email,
  },
  summary: {
    whoWeAre:
      'VIDA (Voluntad Anticipada Digital) es una plataforma que almacena sus directivas médicas anticipadas y las hace accesibles en emergencias.',
    dataCollected:
      'Recopilamos sus datos de identificación (nombre, CURP, correo), datos de salud sensibles (tipo de sangre, alergias, condiciones, medicamentos, directivas), datos de contactos de emergencia y datos de pago procesados por Stripe.',
    whyWeUseIt:
      'Sus datos se utilizan para: (1) mostrar su información médica crítica a servicios de emergencia mediante QR, (2) notificar a sus representantes en emergencias, (3) gestionar su cuenta y suscripción.',
    whoWeShareWith:
      'Compartimos sus datos con: personal médico/emergencias (finalidad principal), sus contactos designados, y proveedores de infraestructura (AWS, Stripe, Resend, WhatsApp Business) bajo medidas de seguridad.',
    yourRights:
      'Puede Acceder, Rectificar, Cancelar u Oponerse al uso de sus datos (derechos ARCO) en "Mi Perfil → Mis Datos y Privacidad" o escribiendo a privacidad@vidadigital.mx.',
    sensitiveData:
      'Sus datos de salud son datos sensibles. Al ingresarlos, otorga consentimiento expreso conforme al Art. 9 LFPDPPP.',
  },
  fullNoticeUrl: '/aviso-privacidad',
};
